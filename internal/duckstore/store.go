...
// process running instead of taking the shard down.
type Store struct {
	cfg StoreConfig
...
func OpenStore(cfg StoreConfig) (*Store, error) {
	...
	s := &Store{cfg: cfg, storageVersion: storageVersion}
...
func (s *Store) Delta() *sql.DB { return s.delta }

// ActiveDeltaGeneration returns the generation of the delta file writes go to.
func (s *Store) ActiveDeltaGeneration() int64 { return s.gen }

// DeltaGenerations returns every valid delta generation present, ascending.
// Older generations hold rows compaction has not consumed yet.
...
// running binary cannot vouch for, resumes the newest valid generation and —
// only when none is valid — creates the next fresh one.
func (s *Store) openDeltas() error {
	...
	if len(s.deltas) == 0 {
		// Every generation present was quarantined (or the directory is
		// fresh). Start the next generation number so a quarantined file is
		// never reused or shadowed.
		gen := maxSeen + 1
		path := filepath.Join(s.cfg.Dir, deltaFileName(gen))
	...
	// for compaction to consume them.
...
// keeping the valid ones for queries and quarantining the rest.
func (s *Store) scanArchives() error {
	...
	sort.Slice(s.windows, func(i, j int) bool {
		if s.windows[i].Tier != s.windows[j].Tier {
			return tierOrder(s.windows[i].Tier) < tierOrder(s.windows[j].Tier)
		}
		return s.windows[i].WindowStart < s.windows[j].WindowStart
	})
	return nil
}
...
// createFile creates a new store file with the given tables and version
// stamp. Delta generations and archive windows are created the same way, so
// every file the store owns carries the same stamp. The database is closed
// again; callers open it through their own handle.
