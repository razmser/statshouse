...
func parseCommandLine() error {
	...
	flag.StringVar(&argv.KHAddr, "kh", "", "clickhouse HTTP address:port, comma-separated; required with --storage-backend=clickhouse and must not be set with --storage-backend=duck")
	...
	flag.StringVar(&argv.DuckStoreDir, "duck-store-dir", aggregator.DefaultConfigAggregator().DuckStoreDir, "duck-store: directory the shard's store owns (delta generations and archive windows are created inside on first start); required with --storage-backend=duck")
	flag.DurationVar(&argv.DuckRetention1s, "duck-retention-1s", aggregator.DefaultConfigAggregator().DuckRetention1s, "duck-store: how long 1s-tier archive window files are kept after the window ended, before being unlinked whole; 0 keeps them forever")
	flag.DurationVar(&argv.DuckRetention1m, "duck-retention-1m", aggregator.DefaultConfigAggregator().DuckRetention1m, "duck-store: how long 1m-tier archive window files are kept after the window ended, before being unlinked whole; 0 keeps them forever")
	...
	flag.IntVar(&argv.DuckQueryConcurrency, "duck-query-concurrency", aggregator.DefaultConfigAggregator().DuckQueryConcurrency, "duck-store: how many store queries execute at once (default max(2, GOMAXPROCS)); a query finding every slot busy waits for one up to the request's own timeout, at most 30s, then is refused as overloaded")
